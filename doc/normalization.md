...
日本語文字をEUC-JPやShift-JISで扱っう場合は、文字を全て２バイト文字に揃えるために、ひらがなやカタカナだけでなく英数字も半角文字を全角文字にする正規化方法が使用されることが多いです。２バイト文字に揃えておくとバイト数割る２で文字列長を計測できます。２バイト文字揃えによる正規化は、主にEUC-JPやShift-JISが日本語文字で使用されていた過去の時代には日本語の文字列処理にPerlが使用されることが多く、PerlのStringはScalaのCharクラスやJavaのchar型のような文字境界を持つオブジェクトのシーケンスという設計にはなっていなかったという背景に起因するのかと思います。<br>
<br>
例：EUC-JPを表す正規表現
```
(
...
POSIX（Portable Operating System Interface）標準に従う文字クラスにすべてのASCII文字を表すASCIIクラスが追加されている。<br>
